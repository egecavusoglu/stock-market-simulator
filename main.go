...
func main(){
	...
	tickerFile := flag.String("ticker-file", "", "Filepath to the custom tickers. Defaults to empty.")
	flag.Parse()

    port := strconv.Itoa(*portPtr)
	writeWelcomeMessage(port)

	// Initial setup
	rand.Seed(*seedPtr)
	timeTicker = time.NewTicker(time.Second)
	stocks = generateStocks(*tickerCountPtr, *tickerFile) // generate the stock market tickers
...
func writeWelcomeMessage(port string){
	fmt.Println(
